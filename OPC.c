...
#define MAX 456976 //amount of passwords you can generate with LEN as 4 (26^4)
	...
	int idx;
	char hashpass;
void iterate(char *str, int current, int end) {
    char c;
    ...
    if (current < (end - 1)) { //if current index is less than length then continue to add characters to string
        for (c = 'A'; c <= 'Z'; ++c) {
            ...
            iterate(str, current + 1, end); //apply recursion to next index
        }
...
int OPC() {
    ...
    iterate(str, 0, LENGTH); //function that will print out all permutations of all capital letters with length set(4) in our case . store in str value
	//need to store all passwords and their hashes
